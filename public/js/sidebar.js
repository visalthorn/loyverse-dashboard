...
const NAV_ITEMS = [
  { page: 'dashboard', href: '/',              icon: 'chart-column',   labelKey: 'nav.dashboard' },
  { page: 'expenses',  href: '/expenses.html',  icon: 'banknote',       labelKey: 'nav.expenses'  },
  { page: 'report',    href: '/report.html',    icon: 'clipboard-list', labelKey: 'nav.reports'   },
  { page: 'summary-report', href: '/summary-report.html', icon: 'files', labelKey: 'nav.summaryReport' },
  { page: 'receipts',  href: '/receipts.html',  icon: 'receipt',        labelKey: 'nav.receipts'  },
  { page: 'staff',     href: '/staff.html',     icon: 'users',          labelKey: 'nav.staff'     },
  { page: 'items',     href: '/items.html',     icon: 'tag',            labelKey: 'nav.items'     },
  { page: 'catalog',   href: '/catalog.html',   icon: 'list',           labelKey: 'nav.catalog'   },
    ...
    <div class="sidebar-header px-5 py-4 border-b border-[color:var(--border)] space-y-2.5" style="position:relative;">
      <div class="sidebar-userrow flex items-center justify-between gap-2">
        <button id="userMenuTrigger" type="button" class="flex items-center gap-2 min-w-0" style="background:none;border:none;cursor:pointer;padding:0;text-align:left;">
          ...
          <span class="user-menu-caret text-[color:var(--text-muted)] flex-shrink-0">${icon('chevron-down', { size: 16 })}</span>
        </button>
        <button onclick="toggleSidebarCollapse()" class="hidden md:inline-flex px-2 py-1 rounded hover:bg-[color:var(--hover-tint)] flex-shrink-0" data-i18n-title="common.collapseSidebar" aria-label="Collapse sidebar">${icon('move-horizontal', { size: 18 })}</button>
      </div>
