...
router.put('/', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  try {
    const before = await pool.query('SELECT * FROM role_permissions WHERE role=$1 AND page=$2', [role, page]);
    ...
    await writeAudit({
      req, action: 'permission_change', entity: 'role_permissions',
      entityId: `${role}:${page}`, before: before.rows[0] || null, after: r.rows[0],
    });
