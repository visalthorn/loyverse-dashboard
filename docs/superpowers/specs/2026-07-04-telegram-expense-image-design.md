...
3. **`parseExpenseImage(caption, imageBase64, referenceDate, anthropicClient)`** — new function in `services/telegramParser.js`, a sibling to the existing `parseExpenseMessage`, not a merged/branching rewrite of it. Both functions share the same `OUTPUT_SCHEMA` (`{ type, date, items[] }`). The system prompt is reworded to acknowledge both input shapes ("...may be text, or a photo of a receipt/invoice...") but stays otherwise the same. A receipt with several line items naturally produces multiple `items` entries — reusing the multi-expense-per-message support the text path already has, with no schema change.

   **Revised (2026-07-04):** originally used `claude-haiku-4-5`, matching the text path, to keep per-message cost negligible. In production, real handwritten receipts (mixed Khmer/English, messy handwriting, phone-camera photos) produced verifiably wrong amounts — Haiku isn't a high-resolution vision model, and misread handwritten digits (confirmed against the receipt's own handwritten total). Switched `parseExpenseImage` only to `claude-sonnet-5` (high-resolution vision, adaptive thinking on) — the text path (`parseExpenseMessage`) stays on `claude-haiku-4-5` unchanged, since that path has shown no accuracy issues and remains the cheaper choice for plain-text messages.
